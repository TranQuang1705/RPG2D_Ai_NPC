...
public class DatabasePickupItem : MonoBehaviour
{
    ...
    {
        Debug.Log($"⏳ {name}: Waiting for database to load...");
    ...
    void InitializeFromDatabase()
    {
        ...
        Debug.Log($"🎯 {name}: Mapped to ItemSO: {runtimeItemSO?.displayName}");
        
        // Cập nhật visual nếu có
        UpdateVisuals();
    ...
    void ExecuteItemEffect(GameObject target)
    {
        switch (databaseItem.effect_type.ToLower())
        {
            case "restore_health":
                ...
                if (health != null)
                {
                    health.HealhPlayer(); // hoặc gọi phương thức heal với amount
